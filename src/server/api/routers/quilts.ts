...
import { createQuiltSchema, updateQuiltSchema, quiltSearchSchema } from '@/lib/validations/quilt';
...
export const quiltsRouter = createTRPCRouter({
  ...
  getAll: publicProcedure.input(quiltSearchSchema.optional()).query(async ({ input }) => {
    try {
  ...
  // Note: Usage management is handled by the dedicated usage router
  // Use api.usage.create, api.usage.end, api.usage.getAllActive, api.usage.getByQuiltId

  // Note: Maintenance records and seasonal recommendations are future features
  // These can be implemented when needed based on user requirements
