...
使用设置脚本生成的密码或在设置页面中配置的密码。
Use the password generated by the setup script or configured in the settings page.

### 环境变量 / Environment Variables

密码现在存储在数据库中，无需环境变量配置。
Passwords are now stored in the database, no environment variables needed.
