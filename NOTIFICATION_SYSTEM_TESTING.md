# 通知系统测试指南

## 📋 概述

本文档提供了完整的通知系统测试步骤，包括数据库迁移、功能测试和验证方法。

## 🔧 前置准备

### 1. 运行数据库迁移

通知系统需要先创建数据库表。有两种方式运行迁移：

#### 方式 A：通过 Neon 控制台（推荐）

1. 登录 [Neon Console](https://console.neon.tech/)
2. 选择你的项目和数据库
3. 打开 SQL Editor
4. 复制 `migrations/007_create_notifications.sql` 的内容
5. 粘贴并执行

#### 方式 B：通过 API 路由

1. 确保应用已部署到 Vercel
2. 访问：`https://your-app.vercel.app/api/migrate/notifications`
3. 查看返回的 JSON 确认迁移成功

### 2. 验证表创建

在 Neon SQL Editor 中运行：

```sql
-- 检查表是否存在
SELECT * FROM information_schema.tables 
WHERE table_name = 'notifications';

-- 查看表结构
\d notifications

-- 检查索引
SELECT indexname, indexdef 
FROM pg_indexes 
WHERE tablename = 'notifications';
```

## 🧪 功能测试

### 测试 1：维护提醒通知

**目的**：测试连续使用超过 30 天的被子会收到维护提醒

**步骤**：

1. 创建一个测试被子
2. 将被子状态改为"使用中"
3. 在数据库中手动修改使用记录的开始时间为 35 天前：

```sql
-- 查找活跃的使用记录
SELECT * FROM usage_records WHERE end_date IS NULL;

-- 修改开始时间为 35 天前
UPDATE usage_records 
SET start_date = NOW() - INTERVAL '35 days'
WHERE id = 'your-usage-record-id';
```

4. 触发通知检查：
   - 刷新应用页面（会自动触发检查）
   - 或者等待 1 小时（自动检查）
   - 或者在浏览器控制台运行：
   ```javascript
   // 手动触发检查
   fetch('/api/trpc/notifications.checkMaintenance', {
     method: 'POST',
     headers: { 'Content-Type': 'application/json' },
     body: JSON.stringify({})
   }).then(r => r.json()).then(console.log);
   ```

5. 点击右上角的通知图标（铃铛）
6. 应该看到一条维护提醒通知，优先级为"中"

**预期结果**：
- ✅ 通知面板显示维护提醒
- ✅ 通知标题：`维护提醒：[被子名称]`
- ✅ 通知内容：`已连续使用 35 天，建议进行清洗或晾晒维护`
- ✅ 优先级徽章显示"中"（黄色）
- ✅ 图标显示扳手 🔧

### 测试 2：淘汰建议通知

**目的**：测试 365 天未使用的被子会收到淘汰建议

**步骤**：

1. 创建一个测试被子
2. 创建一条历史使用记录（已结束）
3. 修改使用记录的结束时间为 400 天前：

```sql
-- 创建历史使用记录
INSERT INTO usage_records (id, quilt_id, start_date, end_date, usage_type, created_at, updated_at)
VALUES (
  gen_random_uuid(),
  'your-quilt-id',
  NOW() - INTERVAL '410 days',
  NOW() - INTERVAL '400 days',
  'REGULAR',
  NOW(),
  NOW()
);
```

4. 触发通知检查（同上）
5. 查看通知面板

**预期结果**：
- ✅ 通知面板显示淘汰建议
- ✅ 通知标题：`淘汰建议：[被子名称]`
- ✅ 通知内容：`已经 400 天未使用，建议考虑是否需要保留`
- ✅ 优先级徽章显示"低"（灰色）
- ✅ 图标显示归档 📦

### 测试 3：通知面板功能

**步骤**：

1. 点击右上角的通知图标
2. 验证通知面板打开
3. 测试以下功能：

#### 3.1 查看通知
- ✅ 未读通知有蓝色背景
- ✅ 未读通知右侧有蓝色圆点
- ✅ 显示优先级徽章
- ✅ 显示相对时间（如"5分钟前"）

#### 3.2 标记已读
- 点击任意通知
- ✅ 蓝色背景消失
- ✅ 蓝色圆点消失
- ✅ 如果有 actionUrl，页面跳转到相关页面

#### 3.3 删除通知
- 点击通知右下角的垃圾桶图标
- ✅ 通知从列表中消失

#### 3.4 全部标记已读
- 点击顶部的"全部标记已读"按钮（双勾图标）
- ✅ 所有通知变为已读状态
- ✅ 未读数量变为 0

#### 3.5 未读计数
- ✅ 通知图标上显示未读数量徽章
- ✅ 数量正确（最多显示 9+）
- ✅ 没有未读时不显示徽章

### 测试 4：自动检查功能

**步骤**：

1. 打开浏览器开发者工具（F12）
2. 切换到 Console 标签
3. 刷新页面
4. 查看控制台输出

**预期结果**：
- ✅ 看到 "Running initial notification check..." 日志
- ✅ 看到 "Notification check completed" 日志
- ✅ 如果创建了新通知，看到 "Created X new notifications"

### 测试 5：通知持久化

**步骤**：

1. 创建一些通知
2. 关闭浏览器
3. 重新打开应用
4. 查看通知面板

**预期结果**：
- ✅ 之前的通知仍然存在
- ✅ 未读状态保持不变
- ✅ 通知按时间倒序排列

## 🔍 数据库验证

### 查看所有通知

```sql
SELECT 
  id,
  type,
  priority,
  title,
  message,
  quilt_id,
  is_read,
  created_at
FROM notifications
ORDER BY created_at DESC
LIMIT 20;
```

### 查看未读通知数量

```sql
SELECT COUNT(*) as unread_count
FROM notifications
WHERE is_read = false;
```

### 查看各类型通知统计

```sql
SELECT 
  type,
  priority,
  COUNT(*) as count,
  SUM(CASE WHEN is_read THEN 0 ELSE 1 END) as unread_count
FROM notifications
GROUP BY type, priority
ORDER BY type, priority;
```

### 手动创建测试通知

```sql
INSERT INTO notifications (
  id, type, priority, title, message, 
  quilt_id, action_url, metadata, created_at, updated_at
)
VALUES (
  gen_random_uuid(),
  'maintenance_reminder',
  'high',
  '测试通知',
  '这是一条测试通知消息',
  NULL,
  '/quilts',
  '{"test": true}',
  NOW(),
  NOW()
);
```

## 🐛 常见问题排查

### 问题 1：通知不显示

**检查清单**：
1. 数据库表是否创建？
   ```sql
   SELECT * FROM notifications LIMIT 1;
   ```
2. 是否有通知数据？
   ```sql
   SELECT COUNT(*) FROM notifications;
   ```
3. 浏览器控制台是否有错误？
4. 网络请求是否成功？（查看 Network 标签）

### 问题 2：通知检查不运行

**检查清单**：
1. 查看浏览器控制台是否有 "Running initial notification check" 日志
2. 检查是否有 JavaScript 错误
3. 确认 NotificationChecker 组件已加载
4. 检查 tRPC 连接是否正常

### 问题 3：未读计数不更新

**解决方法**：
1. 刷新页面
2. 检查 `getUnreadCount` API 是否返回正确数据
3. 在浏览器控制台运行：
   ```javascript
   fetch('/api/trpc/notifications.getUnreadCount')
     .then(r => r.json())
     .then(console.log);
   ```

## 📊 性能测试

### 测试大量通知

创建 100 条测试通知：

```sql
INSERT INTO notifications (id, type, priority, title, message, created_at, updated_at)
SELECT 
  gen_random_uuid(),
  (ARRAY['weather_change', 'maintenance_reminder', 'disposal_suggestion'])[floor(random() * 3 + 1)],
  (ARRAY['high', 'medium', 'low'])[floor(random() * 3 + 1)],
  '测试通知 ' || generate_series,
  '这是测试通知消息 ' || generate_series,
  NOW() - (random() * INTERVAL '30 days'),
  NOW()
FROM generate_series(1, 100);
```

**验证**：
- ✅ 通知面板滚动流畅
- ✅ 加载时间 < 2 秒
- ✅ 分页正常工作（默认显示 50 条）

## 🎯 集成测试场景

### 场景 1：完整的被子使用流程

1. 创建新被子
2. 开始使用（状态改为"使用中"）
3. 修改使用记录为 35 天前
4. 触发通知检查
5. 查看维护提醒通知
6. 点击通知跳转到被子详情
7. 结束使用
8. 标记通知为已读

### 场景 2：通知清理

1. 创建多条通知
2. 标记部分为已读
3. 等待 30 天（或手动修改 created_at）
4. 运行清理：
   ```javascript
   fetch('/api/trpc/notifications.cleanup', {
     method: 'POST',
     headers: { 'Content-Type': 'application/json' },
     body: JSON.stringify({ daysOld: 30 })
   }).then(r => r.json()).then(console.log);
   ```
5. 验证旧的已读通知被删除

## 📝 测试检查清单

### 基础功能
- [ ] 数据库表创建成功
- [ ] 可以创建通知
- [ ] 可以查看通知列表
- [ ] 可以标记为已读
- [ ] 可以删除通知
- [ ] 未读计数正确

### 通知规则
- [ ] 维护提醒（30天）正常触发
- [ ] 淘汰建议（365天）正常触发
- [ ] 不会创建重复通知

### UI/UX
- [ ] 通知面板正常打开/关闭
- [ ] 优先级徽章显示正确
- [ ] 图标显示正确
- [ ] 时间显示正确
- [ ] 点击跳转正常
- [ ] 加载状态显示
- [ ] 空状态显示

### 性能
- [ ] 大量通知时性能良好
- [ ] 自动检查不影响用户体验
- [ ] API 响应时间 < 1 秒

## 🚀 生产环境验证

部署到生产后：

1. 访问应用
2. 打开通知面板
3. 验证所有功能正常
4. 检查 Vercel 日志是否有错误
5. 监控数据库查询性能

## 📞 需要帮助？

如果遇到问题：
1. 查看浏览器控制台错误
2. 查看 Vercel 部署日志
3. 查看 Neon 数据库日志
4. 检查本文档的常见问题部分
